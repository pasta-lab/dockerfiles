# Based on https://hub.docker.com/r/zjuchenyuan/aflgo
FROM zjuchenyuan/base

RUN apt-get update && \
  apt-get install -y sudo curl wget build-essential make cmake ninja-build git subversion python2.7 binutils-gold binutils-dev python-dev python3 python3-dev python3-pip autoconf automake libtool-bin python-bs4 libclang-4.0-dev gawk pkg-config libboost-all-dev gnuplot &&\
...
RUN mkdir -p /build && cd /build &&\
  ...
  tar xf llvm-4.0.0.src.tar.xz --no-same-owner && tar xf cfe-4.0.0.src.tar.xz --no-same-owner && tar xf compiler-rt-4.0.0.src.tar.xz --no-same-owner && tar xf libcxx-4.0.0.src.tar.xz --no-same-owner && tar xf libcxxabi-4.0.0.src.tar.xz --no-same-owner &&\
  rm *.tar.xz &&\
